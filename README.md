...
El SPI toma la onda de pulso periferica y extrae el heart beat rate. y la amplitud de cada pulso para luego hacer una equivalencia de 0 a 100, el ANI mide es el ritmo del corazon de latido a latido y el indice de perfusion la relacion entre la onda pulsatil del ppg y la parte de valle.
El SPI al igual que el ANI y el índice de perfusión comparten el objetivo de monitorear activaciones nociceptivas no controladas por la anestesia, pero cada uno lo hace a partir de una señal distinta, siendo medidores de demuestran como distintos enfoques de captura para la misma señal pueden ser interpretados de diferentes maneras y aun así servir a el mismo propósito. 

# Análisis y Discusión De Resultados

<img width="1600" height="1393" alt="image" src="https://github.com/user-attachments/assets/64e31a5f-e4fa-47b9-9e46-068ea1497b6f" />

Se puede observar el conteo  crudo de la señal MAX30102 ya invertido donde se pueden ver oscilaciones rápidas y resalta un aumento progresivo de la base de la señal a lo largo del tiempo reflejando la importancia del procesamiento de la señal para una buena interpretación, luego podemos observar la señal ya procesada, donde se ve mayor estabilidad en la señal, manteniéndose entre los 800u.a y 1200u.a, y finalmente el SPI durante el periodo de relajación, la técnica del  hielo y el periodo de recuperación que complementa correctamente la lectura del PPG de arriba, podemos observar el  periodo de adaptación de 15 segundos antes de la captura de el SPI donde aun no se inicia ni maniobra ni captura, y luego un promedio de SPI de 40, tras iniciar la maniobra de el hielo o el estimulo doloroso, aumentan  la variabilidad del offset de las oscilaciones y se muestran reducciones de intervalos entre cada pulso además de una reducción de la amplitud de estos, mostrando el proceso de vasoconstricción que se busca provocar,  y evidenciándolo en el aumento del SPI en esos intervalos, durante esta prueba el paciente, debido a la temperatura manejada y su respuesta a ella, decidió retirar la mano a los 18 segundos por lo que se puede observar el proceso en el que se retoma la dominancia parasimpática y empieza el proceso de vasodilatación hacia el diámetro. aunque el movimiento de las extremidades durante la captura con la técnica para estimular el tono simpático y la quietud causada por el alivio tras la técnica también son variables a tomar en cuenta para la gran diferencia entre el durante y el reposo

# Conclusión

La monitorización de la nocicepción durante los procedimiento, siendo un problema aun latente por la imposibilidad actual para evaluarlo directamente en un paciente sedado, el índice plestimografico quirúrgico como medida para conocer la respuesta nociceptiva de forma controlada demostró ser una opción viable usando un sensor óptico de bajo costo y placa de desarrollo protoboard de uso general, el sistema fue capaz de capturar la señal PPG, extraer sus componentes como de intervalo, amplitud y con ellas calcular el SPI en tiempo real usando un algoritmo de detección de picos de la literatura, el cual tuvo una respuesta positiva frente al test de compresión de frio al demostrar efectivamente un incremento en el SPI presentando coherencia con el aumento de la dominancia simpática en el sistema autónomo, sin embargo aun es necesario una evaluación de estímulos y periodos de relajación para conocer la efectividad entre múltiples seguidos, pues durante el laboratorio, minutos después de un estimulo se demostró una imposibilidad de lectura ante el inicio de esta en un estado ya con el sistema simpático dominante y no  lectura durante una prueba con múltiples estímulos del mismo tipo en el intervalo de tiempo, dejando claro  el punto de enfoque hacia el mejoramiento de la técnica y la compresión de sus limites. 
